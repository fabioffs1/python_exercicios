# João Papo-de-Pescador, homem de bem, comprou um microcomputador 
# para controlar o rendimento diário de seu trabalho. 
# Toda vez que ele traz um peso de peixes maior que o estabelecido pelo 
# regulamento de pesca do estado de São Paulo (50 quilos) deve pagar uma multa de
# R$ 4,00 por quilo excedente. João precisa que você faça um programa que 
# leia a variável peso (peso de peixes) e calcule o excesso. 
# Gravar na variável excesso a quantidade de quilos além do limite e na 
# variável multa o valor da multa que João deverá pagar. 
# Imprima os dados do programa com as mensagens adequadas.
peso = float(input('Informe a quantidade de peixes em kg: '))
limite_regulamento = 50.00
peso_excedente = peso - limite_regulamento
multa_por_quilo = 4.00
valor_multa = peso_excedente * multa_por_quilo
print(f'\nQuantidade pescada: {peso:.2f} kg')
print(f'Peso excedente: {peso_excedente:.2f} kg')
print(f'Valor da multa: R$ {valor_multa:.2f}')